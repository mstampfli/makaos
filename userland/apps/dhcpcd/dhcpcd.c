// dhcpcd — MakaOS DHCP client daemon (RFC 2131)
//
// A foreground daemon that obtains an IPv4 lease for the primary NIC,
// programs the kernel via net_ifconfig(), writes /etc/resolv.conf, then
// sleeps until T1 and renews.  No dependencies beyond libc.
//
...
static void log_ip(const char* prefix, uint32_t ip_be) {
    ...
    int n = snprintf(buf, sizeof(buf), "[dhcpcd] %s %s\n", prefix, ipstr);
...
int main(int argc, char** argv) {
...

    {
        ...
        int n = snprintf(buf, sizeof(buf),
                         "[dhcpcd] MAC %02x:%02x:%02x:%02x:%02x:%02x\n",
